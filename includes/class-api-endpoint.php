...
class SFTP_Sync_GS_API_Endpoint {
    ...
    public function verify_api_key($request) {
        ...
        if (!$this->check_rate_limit()) {
            SFTP_Sync_GS::log('Rate limit exceeded from ' . $this->get_client_ip(), 'error');
        ...
        if (empty($stored_key)) {
            SFTP_Sync_GS::log('API key not configured', 'error');
            return false;
        }
        ...
        if (empty($received_key)) {
            SFTP_Sync_GS::log('No API key provided in request from ' . sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? 'unknown')), 'error');
            return false;
        }
        
        if (!hash_equals($stored_key, $received_key)) {
            SFTP_Sync_GS::log('Invalid API key from ' . sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'] ?? 'unknown')), 'error');
            return false;
        }
    ...
    public function handle_upload($request) {
        ...
        SFTP_Sync_GS::log('Upload request received from ' . $client_ip, 'info');
        ...
        if (empty($file_content)) {
            SFTP_Sync_GS::log('No file content provided', 'error');
            return new WP_REST_Response([
                'success' => false,
        ...
        if ($result['success']) {
            ...
            SFTP_Sync_GS::log('Upload failed: ' . $result['error'], 'error');
    ...
    public function handle_status($request) {
        ...
        return new WP_REST_Response([
            'status' => 'active',
            'plugin_version' => SFTP_SYNC_GS_VERSION,
